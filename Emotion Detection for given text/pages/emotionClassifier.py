...
pipe_lr = joblib.load(open("models/Emotion detection.pkl", "rb"))
    ...
    if submit:
        ...
        if text:
            ...
            with col1:
                ...
                st.success(
                    f"Emotion Predicted : {prediction.upper()} {emoji_icon}")
            ...
            if prdcts and txts and probas:
                st.write("Data Frame")
                d = {'Text': txts, 'Emotion': prdcts,
                     'Probability': probas, 'Date': dateUser}
                df = pd.DataFrame(d)
                st.write(df)
                ...
                index_emotion = st.session_state.emotions.index(
                    prediction.upper())
                st.session_state.occurence[index_emotion] += 1

                d_pie = {'Emotion': st.session_state.emotions,
                         'Occurence': st.session_state.occurence}
